...
import type { DxfSolidCell, MapType, MarkerIconRule } from "@/types/map";
...
import {
  createFilteredRoads,
  fetchBuildingsInsideArea,
  fetchRawRoadsInsideArea,
} from "@/utils/osm";
import {
  createDxfBlockNameFromPrefix,
  imageFileToDxfCells,
} from "@/utils/imageToDxfBlock";

export const useMapExporter = () => {
  const [geoData, setGeoData] = useState<any>(null);
  const [visibleGeoData, setVisibleGeoData] = useState<any>(null);
  const [buildingData, setBuildingData] = useState<any>(null);
  const [roadData, setRoadData] = useState<any>(null);
  const [rawRoadData, setRawRoadData] = useState<any>(null);
  const [polygonAreas, setPolygonAreas] = useState<any>(null);
  ...
  const [markerIconRules, setMarkerIconRules] = useState<MarkerIconRule[]>([]);
  const [markerNamePrefix, setMarkerNamePrefix] = useState("");
  const [markerIconUrl, setMarkerIconUrl] = useState("");
  const [markerDxfCells, setMarkerDxfCells] = useState<DxfSolidCell[]>([]);

  const [roadWidthMeter, setRoadWidthMeter] = useState("");
  const [detectedRoadWidthMeter, setDetectedRoadWidthMeter] = useState<
    number | null
  >(null);
  ...
  const renderGeoJsonInChunks = (converted: any) => {
    setIsLoading(true);
  ...
  const handleUploadKmlKmz = async (
  ...
  const handleUploadMarkerIcon = async (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = event.target.files?.[0];
  ...
  const handleAddMarkerIconRule = () => {
    ...
    setMarkerIconRules((prev) => {
      const filtered = prev.filter(
        (item) => item.namePrefix.toLowerCase() !== prefix.toLowerCase(),
      );
  ...
  const handleRemoveMarkerIconRule = (namePrefix: string) => {
    setMarkerIconRules((prev) =>
      prev.filter((item) => item.namePrefix !== namePrefix),
    );
  };

  const loadBuildingsAndRoads = async (
    bounds: L.LatLngBounds,
    currentPolygonAreas: any,
  ) => {
    ...
    try {
      const buildings = await fetchBuildingsInsideArea(
        bounds,
        currentPolygonAreas,
      );
    ...
    try {
      ...
      const { filteredRoads, firstDetectedWidth } = createFilteredRoads(
        rawRoads,
        currentPolygonAreas,
        getCustomRoadWidth(),
      );
  ...
  const fitMapAndLoadOsmData = () => {
    if (!visibleGeoData || hasFitBoundsRef.current) return;
