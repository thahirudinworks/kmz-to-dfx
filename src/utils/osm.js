...
import { filterBuildingsInsidePolygons, filterRoadPolygonsInsidePolygons, } from './geojson';
export const parseOsmWidth = (widthValue) => {
    if (!widthValue)
        return null;
...
export const getRoadWidthMeterFromOsm = (properties) => {
...
export const buildRoadPolygonGeojson = (rawRoadGeojson, customWidthMeter) => {
    let firstDetectedWidth = null;
...
export const fetchBuildingsInsideArea = async (bounds, polygonGeojson) => {
...
export const fetchRawRoadsInsideArea = async (bounds) => {
    const south = bounds.getSouth();
    const west = bounds.getWest();
    const north = bounds.getNorth();
    const east = bounds.getEast();
...
export const createFilteredRoads = (rawRoadGeojson, polygonGeojson, customWidthMeter) => {
    const { roadPolygonGeojson, firstDetectedWidth } = buildRoadPolygonGeojson(rawRoadGeojson, customWidthMeter);
    return {
        filteredRoads: filterRoadPolygonsInsidePolygons(roadPolygonGeojson, polygonGeojson),
        firstDetectedWidth,
    };
};
