const rgbToAcadColor = (r, g, b) => {
    ...
    if (g > 160 && r < 140 && b < 140)
        return 3;
    if (g > 150 && b > 150 && r < 140)
        return 4;
    if (b > 160 && r < 140 && g < 140)
        return 5;
    if (r > 150 && b > 150 && g < 140)
        return 6;
    return 8;
};
const sanitizeBlockName = (value) => {
    const clean = value.replace(/[^a-zA-Z0-9_]/g, '_').toUpperCase();
    return `ICON_${clean || 'MARKER'}`;
};
export const imageFileToDxfCells = async (file, options) => {
